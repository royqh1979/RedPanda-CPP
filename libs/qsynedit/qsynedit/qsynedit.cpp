...
{
    mUndoList->addChange(ChangeReason::Selection,mSelectionBegin,mSelectionEnd,QStringList(), activeSelectionMode());
    selectAll();
    setSelText(text);
}
...
int QSynEdit::charToGlyphLeft(int line, int charPos) const
{
    ...
    return mDocument->charToGlyphStartPosition(line, s, charPos);
}
...
CharPos QSynEdit::getPreviousLeftBrace(int ch, int line)
{
    QChar testCh;
    QString vDummy;
    PTokenAttribute attr;
    ...
    if (posY<0)
        return invalidPos;
    QString s = mDocument->getLine(posY);
    if ((posX >= s.length()) || (posX<0))
        posX = s.length() - 1;
    int numBrackets = 1;
    while (true) {
        while (posX<0){
...
CharPos QSynEdit::nextWordChar(const CharPos &pos) const
{
    return findNextChar(pos, CharType::WordChar);
}

CharPos QSynEdit::nextNonWordChar(const CharPos &pos) const
{
    return findNextChar(pos, CharType::NonWordChar);
}

CharPos QSynEdit::prevWordChar(const CharPos &pos) const
{
    return findPrevChar(pos, CharType::WordChar);
}

CharPos QSynEdit::prevNonWordChar(const CharPos &pos) const
{
    return findPrevChar(pos, CharType::NonWordChar);
}
...
CharPos QSynEdit::ensureBufferCoordValid(const CharPos &coord)
{
    int nMaxX;
    CharPos value = coord;
    if (value.line > mDocument->count()-1)
...
void QSynEdit::insertLine(bool moveCaret)
{
    ...
    reparseLines(mCaretY, mCaretY+1, false, false);
    ...
    mDocument->insertLine(mCaretY+1, indentSpacesForRightLineText+rightLineText);
    nLinesInserted++;
    ...
    if (!mUndoing) {
        ...
        if (!notInComment && (!syntaxer()->blockCommentBeginSymbol().isEmpty())
            ...
            mDocument->insertLine(mCaretY+1, indentSpacesForRightLineText);
            nLinesInserted++;
...
QRect QSynEdit::calculateCaretRect() const
{
    ...
    if (!mInputPreeditString.isEmpty()) {
        ...
        if (sLine == mGlyphPostionCacheForInputMethod.str)  {
            int glyphIdx = searchForSegmentIdx(mGlyphPostionCacheForInputMethod.glyphCharList,0,sLine.length(),mCaretX+mInputPreeditString.length());
...
{
    internalSetCaretXY(CharPos{x,y}, ensureVisible);
}
...
{
    internalSetCaretXY(CharPos{mCaretX,value});
}
...
void QSynEdit::foldOnLinesDeleted(int line, int count, bool &needRescan)
{
    needRescan = false;
    // Delete collapsed inside selection
    for (int i = mAllFoldRanges->count()-1;i>=0;i--) {
...
int QSynEdit::doInsertTextByNormalMode(const CharPos& pos, const QStringList& text, CharPos &newPos)
{
    QString sLeftSide;
    QString sRightSide;
    QString str;
...
//    int SpaceCount;
    ...
    if (text.length()>1) {
        ...
        mDocument->insertLines(caretY+1, text.length()-1);
    } else {
        str = sLeftSide + text[0] + sRightSide;
        properSetLine(caretY, str, false);
    }
    reparseLines(caretY,caretY+1, false, false);
    timer.start();
...
void QSynEdit::onLinesDeleted(int line, int count)
{
    ...
    if (useCodeFolding())
        foldOnLinesDeleted(line, count, needRescanFold);
    if (mSyntaxer->needsLineState()) {
        reparseLines(line, line + count, needRescanFold);
    }
...
{
    if (useCodeFolding())
        foldOnLinesInserted(line, count);
    if (mSyntaxer->needsLineState()) {
...
CharPos QSynEdit::selEnd() const
{
    if (mActiveSelectionMode==SelectionMode::Column)
        return mSelectionEnd;
    if ((mSelectionEnd.line < mSelectionBegin.line)
      || ((mSelectionEnd.line == mSelectionBegin.line) && (mSelectionEnd.ch < mSelectionBegin.ch)))
...
void QSynEdit::setBlockEnd(CharPos value)
{
    ...
    if (mActiveSelectionMode == SelectionMode::Normal) {
      if (value.line >= 0 && value.line < mDocument->count())
          value.ch = std::min(value.ch, getDisplayStringAtLine(value.line).length());
      else
          value.ch = 0;
    }
    value.ch = std::max(value.ch, 0);
...
void QSynEdit::setSelBegin(CharPos value)
{
    ...
    if (selAvail()) {
        ...
        SelChanged =
          (mSelectionBegin.ch != value.ch) || (mSelectionBegin.line != value.line) ||
          (mSelectionEnd.ch != value.ch) || (mSelectionEnd.line != value.line);
        mSelectionBegin = value;
        mSelectionEnd = value;
    }
    if (SelChanged)
