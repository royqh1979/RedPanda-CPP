...
QSet<QString> NASMSyntaxer::InstructionNames;
QMap<QString,QString> NASMSyntaxer::Instructions;

const QSet<QString> NASMSyntaxer::Registers {
    "ah","al","ax","eax",
    "bh","bl","bx","ebx",
    "ch","cl","cx","ecx",
    "dh","dl","dx","edx",
...
NASMSyntaxer::NASMSyntaxer()
{
    ...
    //mPreprocessorAttribute = std::make_shared<TokenAttribute>(SYNS_AttrPreprocessor, TokenType::Keyword);
    mPreprocessorAttribute = std::make_shared<TokenAttribute>(SYNS_AttrPreprocessor,
                                                                    TokenType::Preprocessor);
    addAttribute(mPreprocessorAttribute);
}

const PTokenAttribute &NASMSyntaxer::numberAttribute() const
{
    return mNumberAttribute;
}

const PTokenAttribute &NASMSyntaxer::registerAttribute() const
{
    return mRegisterAttribute;
}

const PTokenAttribute &NASMSyntaxer::preprocessorAttribute() const
{
    return mPreprocessorAttribute;
}

void NASMSyntaxer::procComment()
{
...
void NASMSyntaxer::procIdent(IdentPrefix prefix)
{
    int start = mRun;
...
void NASMSyntaxer::procLF()
{
    mTokenID = TokenId::Space;
    mRun++;
}

void NASMSyntaxer::procLower()
{
    mRun++;
    mTokenID = TokenId::Symbol;
...
void NASMSyntaxer::procNull()
{
    mTokenID = TokenId::Null;
}

void NASMSyntaxer::procNumber()
{
    mRun++;
...
void NASMSyntaxer::procSingleQuoteStringProc()
{
...
void NASMSyntaxer::procSlashProc()
{
    mRun++;
    if (mLine[mRun] == '/') {
      mTokenID = TokenId::Comment;
...
void NASMSyntaxer::procSymbol()
{
    mRun++;
    mTokenID = TokenId::Symbol;
}

void NASMSyntaxer::procUnknown()
{
    mRun++;
    mTokenID = TokenId::Unknown;
}

bool NASMSyntaxer::isIdentStartChar(const QChar &ch) const
{
    if (ch == '_') {
        return true;
    }
...
QString NASMSyntaxer::getToken() const
{
    return mLineString.mid(mTokenPos,mRun-mTokenPos);
}

const PTokenAttribute &NASMSyntaxer::getTokenAttribute() const
{
    switch(mTokenID) {
...
int NASMSyntaxer::getTokenPos()
{
    return mTokenPos;
}

void NASMSyntaxer::next()
{
    mTokenPos = mRun;
...
void NASMSyntaxer::setLine(const QString &newLine, int lineNumber)
{
    mLineString = newLine;
...
bool NASMSyntaxer::isCommentNotFinished(int /*state*/) const
{
    return false;
}

bool NASMSyntaxer::isStringNotFinished(int /*state*/) const
{
    return false;
}

SyntaxState NASMSyntaxer::getState() const
{
    SyntaxState state;
...
void NASMSyntaxer::setState(const SyntaxState&)
{
    mHasTrailingSpaces = false;
}

void NASMSyntaxer::resetState()
{
    mHasTrailingSpaces = false;
}

bool NASMSyntaxer::supportFolding()
{
    return false;
}

bool NASMSyntaxer::needsLineState()
{
    return true;
}
...
QSet<QString> NASMSyntaxer::keywords()
{
    if (mKeywordsCache.isEmpty()) {
        ...
        //mKeywordsCache.unite(PreprocessorDirectives);
    }
    return mKeywordsCache;
}

const PTokenAttribute &NASMSyntaxer::directiveAttribute() const
{
    return mDirectiveAttribute;
}

const PTokenAttribute &NASMSyntaxer::labelAttribute() const
{
    return mLabelAttribute;
}

QString NASMSyntaxer::commentSymbol()
{
    return ";";
}
}
