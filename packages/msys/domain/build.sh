...
function build() {
  ...
  [[ "$_NATIVE_ARCH" == "i686" ]] || [[ -d "$_PKGDIR/mingw64" ]] || cp -r "mingw64" "$_PKGDIR"
...
function package() {
  ...
  if [[ _7Z_REPACK -eq 1 ]]; then
    7z x "RedPandaCPP-$_REDPANDA_VERSION-$_DISPLAY_ARCH-User.exe" -o"RedPanda-CPP" -xr'!$PLUGINSDIR'
    7z a -t7z -mx=9 -ms=on -mqs=on -mf=BCJ2 -m0="LZMA2:d=$_7Z_DICT_SIZE:fb=273:c=2g" "RedPandaCPP-$_REDPANDA_VERSION-$_DISPLAY_ARCH.7z" "RedPanda-CPP"
    rm -rf "RedPanda-CPP"
  fi
  popd
}
...
function dist() {
  cp "$_PKGDIR/RedPandaCPP-$_REDPANDA_VERSION-$_DISPLAY_ARCH-System.exe" "$_DISTDIR"
  cp "$_PKGDIR/RedPandaCPP-$_REDPANDA_VERSION-$_DISPLAY_ARCH-User.exe" "$_DISTDIR"
  [[ _7Z_REPACK -eq 1 ]] && cp "$_PKGDIR/RedPandaCPP-$_REDPANDA_VERSION-$_DISPLAY_ARCH.7z" "$_DISTDIR"
}
...
download-assets
prepare-mingw 32
[[ "$_NATIVE_ARCH" == "i686" ]] || prepare-mingw 64
prepare-llvm-mingw
prepare-openconsole
build
package
